//used Zamansky's instead of rewriting my own.

var rawschooldata = rawdata.data;
...
var rawmath = scores.map(function(x){return x.math}).map(function(x){return parseInt(x)}).filter(function(x){return !isNaN(x)});

//can't install underscore.js
//var rawmath = _.filter(_.map(_.map(scores,function(x){return x.math}),function(x){parseInt(x)}),function(x){return !isNaN(x);
